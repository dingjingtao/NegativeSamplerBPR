...
/**
 * Implement BPR with view data (BPR+View_loss_beta).
 * Ding, Yu, et all. "Sampler Design for Bayesian Personalized Ranking by Leveraging View Data"
 * 
 * @author dingjingtao
 *
 */
public class bpr_plusview_loss_beta extends TopKRecommender {
	/** Model priors to set. */
	...
	public bpr_plusview_loss_beta(SparseMatrix trainMatrix, ArrayList<Rating> testRatings,
			...
			int showbound ,int showtime ,SparseMatrix viewMatrix,double[] user_ratios,double alpha) {
		...
		this.user_ratios = user_ratios.clone();
		this.alpha = alpha;
	}
	...
	private void update_uij(int u, int i, int j) {
		...
		double omega_adv = Math.pow(user_ratios[u],alpha) / (Math.pow(user_ratios[u],alpha) + 1);
		for (int f = 0; f < factors; f ++) {
